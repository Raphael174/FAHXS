...
class combustorProp:
    ...
    exhaust_diameter : float = 24e-3        # [m] nozzle throat diameter (= 30mm - 6mm)
    ...
    Dh_coil : float = 7e-3             # [m] coil inner hydraulic diameter
    thickness_coil_wall : float = 0.5e-3    # [m] coil tube wall thickness (250 MPa 316L @ 90 bar, SF 1.15)
    gap_shell2coil : float = 22.5e-3       # [m] radial gap between outer coil face and shell wall
    length_2_coil : float = 0e-3            # [m] axial end-clearance between coil and shell ends
    ...
    Nusselt_correction : float = 0.2       # user tuning factor applied on top of Nu_shell
    ...
    # Roughness
    combustor_roughness : float = 1e-6      # [m] shell-side surface roughness
    channel_roughness : float = 1e-6        # [m] coil inner surface roughness
...
class numericalProp:
    ...
    N_arc_steps_per_turn : int = 100      # sub-steps per coil turn; 1 = one full-turn per march step (default)
